...
bool equal(QU p,QU q)
{
	if(p.front!=q.front||p.rare!=q.rare)
		return(false);
	while(!isempty(p))
	{
		if(dequeue(&p)!=dequeue(&q))
			return false;
	}
	return true;
}
	...
	do
	{
		...
		switch(c)
		{
			...
			case 5:
				...
				do
				{
					printf("Enter the number of 1st queue = ");
					scanf("%d",&n);
					if(n>count)
						printf("System has only %d number of queue.\nTry again\n\n",count);
				}while(n>count);
				do
				{
					printf("Enter the number of 2nd queue = ");
					scanf("%d",&x);
					if(x>count)
						printf("System has only %d number of queue.\nTry again\n\n",count);
				}while(x>count);
				if(equal(q[n-1],q[x-1])==true)
